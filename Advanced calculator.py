...
def btnEquals():
    ...
    try:
        result = str(eval(val))
        data.set(result)
        val = result  # so you can continue calculating with the result
    except:
        data.set("Error")
        val = ""
...
# Row 5
btn_square = Button(root, text="x²", font=("Arial", 12, "bold"), bd=12, height=2, width=6, command=btnSquare)
btn_square.grid(row=5, column=0)
btn_sqrt = Button(root, text="√", font=("Arial", 12, "bold"), bd=12, height=2, width=6, command=btnSqrt)
btn_sqrt.grid(row=5, column=1)
btn_percent = Button(root, text="%", font=("Arial", 12, "bold"), bd=12, height=2, width=6, command=btnPercent)
